...
class AccessLogReport(object):
    ...
    @time_range_decorator
    def particular_ip_report(self, ip, show_graph=False):
        """
            i/p: 
        ...
        devices_used = set()
        urls_hit = {}
        log_list = [i for i in self.lines if ip in i]
        for item in log_list:
            self.overall_helper(item, urls_hit=urls_hit)
            try:
    ...
    def overall_helper(self, item, ip_address=None, urls_hit=None):
        separated_by_space = item.split(' ')
